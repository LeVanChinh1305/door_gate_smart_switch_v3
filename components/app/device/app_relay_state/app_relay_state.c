...
static relay_state_changed_cb_t g_fnCallbacks[DF_MAX_RELAY_STATE_CALLBACKS] = {
    0};
...
esp_err_t app_relay_state_Init(void) {
  ...
  ESP_LOGI(
      TAG,
      "Khởi tạo module app_relay_state thành công. Trạng thái ban đầu: CLOSED");
  return ESP_OK;
}

const char *app_relay_state_ToString(e_relay_state_t eState) {
  switch (eState) {
  case E_RELAY_STATE_STOPPED:
    return "STOPPED";
  case E_RELAY_STATE_OPENING:
    return "OPENING";
  case E_RELAY_STATE_CLOSING:
    return "CLOSING";
  case E_RELAY_STATE_OPENED:
    return "OPENED";
  case E_RELAY_STATE_CLOSED:
    return "CLOSED";
  case E_RELAY_STATE_OBSTACLE:
    return "OBSTACLE";
  case E_RELAY_STATE_SAFETY_LOCKED:
...
esp_err_t
app_relay_state_RegisterCallback(relay_state_changed_cb_t fnCallback) {
  if (fnCallback == NULL) {
    return ESP_ERR_INVALID_ARG;
  }
  ...
  if (xSemaphoreTake(g_hStateMutex, pdMS_TO_TICKS(100)) == pdPASS) {
    ...
    if (g_u8CallbackCount < DF_MAX_RELAY_STATE_CALLBACKS) {
      ...
      ESP_LOGI(TAG, "Đã đăng ký callback thay đổi trạng thái relay (index=%d)",
               g_u8CallbackCount - 1);
    } else {
      ESP_LOGW(TAG, "Danh sách callback đầy (%d)",
               DF_MAX_RELAY_STATE_CALLBACKS);
      ret = ESP_ERR_NO_MEM;
    }
    xSemaphoreGive(g_hStateMutex);
  } else {
