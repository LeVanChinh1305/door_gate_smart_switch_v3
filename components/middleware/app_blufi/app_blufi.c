...
// 
void app_blufi_gap_event_handler(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param)
{
    switch (event) {
...
void esp_blufi_gatt_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    switch (event) {
...
static void blufi_event_callback(esp_blufi_cb_event_t event, esp_blufi_cb_param_t *param)
{
    ...
    case ESP_BLUFI_EVENT_REQ_CONNECT_TO_AP: {
        ...
        // 1. Xóa cấu hình Wi‑Fi cũ trước khi lưu AP mới, tránh dùng subnet/SSID cũ sau khi đổi mạng
            ...
            if ((cmd_name != NULL) && cJSON_IsString(cmd_name) && (cmd_name->valuestring != NULL)) {
                ...
                if (strcmp(cmd_name->valuestring, "CmdGetDeviceID") == 0) {
                    ...
                    if (cRequestedDeviceAddress[0] == '\0') {
                        ...
                        (void)snprintf(cRequestedDeviceAddress, sizeof(cRequestedDeviceAddress), "%02X%02X%02X%02X%02X%02X",
                                       mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);
                    }
...
esp_err_t app_blufi_Init(void)
{
    ...
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "%s Khởi tạo bộ điều khiển BT thất bại: %s", __func__, esp_err_to_name(ret));
        return ret;
    }
    ...
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "%s Kích hoạt bộ điều khiển BT thất bại: %s", __func__, esp_err_to_name(ret));
        return ret;
    }
    ...
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "%s Khởi tạo Bluetooth Stack (Bluedroid) thất bại: %s", __func__, esp_err_to_name(ret));
        return ret;
    }
    ...
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "%s Bật Bluetooth Stack (Bluedroid) thất bại: %s", __func__, esp_err_to_name(ret));
        return ret;
    }

    // Set tên thiết bị (dùng Base MAC của chip) 
    uint8_t mac[6];
    esp_read_mac(mac, ESP_MAC_BASE); // Đọc địa chỉ MAC định danh phần cứng được ghi sẵn trong eFuse của chip ESP32. ESP_MAC_BASE: Lấy địa chỉ MAC cơ sở (Base MAC)
    char device_name[30]; 
    snprintf(device_name, sizeof(device_name), "VCONNEX-%02X%02X", mac[4], mac[5]); // ghép chuỗi 

    // Thiết lập tên thiết bị cho tầng GAP BLE
    ret = esp_ble_gap_set_device_name(device_name);
    if (ret) {
        ESP_LOGE(TAG, "%s Thiết lập tên thiết bị thất bại: %s", __func__, esp_err_to_name(ret));
        return ret;
    }

    // === Đăng ký GAP callback ===
    // Đăng ký Handler xử lý sự kiện GAP (Generic Access Profile)
    // Khi có các sự kiện BLE ở cấp độ kết nối/quảng bá xảy ra (như bắt đầu/dừng quảng bá, có thiết bị kết nối/ngắt kết nối, cập nhật tham số kết nối
    ret = esp_ble_gap_register_callback(app_blufi_gap_event_handler); 
    if (ret) {
        ESP_LOGE(TAG, "%s Đăng ký GAP callback thất bại: %s", __func__, esp_err_to_name(ret));
        return ret;
    }
    ...
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "%s Đăng ký GATT callback thất bại: %s", __func__, esp_err_to_name(ret));
        return ret;
    }
    ...
    if (ret) {
        ESP_LOGE(TAG, "%s blufi profile init failed, error code = %x", __func__, ret);
        return ret;
    }
