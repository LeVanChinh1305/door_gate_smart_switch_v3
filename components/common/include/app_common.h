...
//=========================================================================================================
// ép kiểu biến không dùng thành void để tránh cảnh báo compiler
#define DF_UNUSED(x) ((void)(x))

// số lượng phần tử trong mảng tĩnh: sizeof(mảng)/sizeof(phần tử đầu tiên), lưu ý: không dùng được với con trỏ
#define DF_ARRAY_SIZE(x) (sizeof(x) / sizeof((x)[0]))

// giá trị nhỏ nhất giữa hai giá trị
#define DF_MIN(a, b) (((a) < (b)) ? (a) : (b))

// giá trị lớn nhất giữa hai giá trị
#define DF_MAX(a, b) (((a) > (b)) ? (a) : (b))

// ép x về khoảng [min, max]
#define DF_CLAMP(x, min, max) (DF_MIN(DF_MAX((x), (min)), (max)))
...
// dùng cho OTA
#define DF_TASK_PRIO_MAX        (6)
// relay_task — lệnh dừng/đảo chiều cửa
#define DF_TASK_PRIO_CRITICAL   (5)
// touch_task — đọc phím chạm
#define DF_TASK_PRIO_HIGH       (4)
// udp_task, VÀ mqtt_cfg.task.priority (set tường minh để tránh trùng prio=5 với relay_task)
#define DF_TASK_PRIO_NORMAL     (3)
// led_task, buzzer_task
#define DF_TASK_PRIO_LOW        (1)

// Task Stack Size
// siêu nhỏ
#define DF_TASK_STACK_MIN       (1024)
// Dành cho task cơ bản, ít biến cục bộ (điều khiển Relay, hiệu ứng LED)
#define DF_TASK_STACK_SMALL     (2048)
// Dành cho task logic vừa phải, xử lý mảng dữ liệu nội bộ hoặc chuỗi ngắn
#define DF_TASK_STACK_MEDIUM    (3072)
// Dành cho task giao tiếp mạng cơ bản (BluFi, MQTT, UDP) không mã hóa SSL
#define DF_TASK_STACK_NETWORK   (4096)
// Dành cho task xử lý cấu trúc dữ liệu phức tạp (phân tích chuỗi JSON lớn, cJSON)
#define DF_TASK_STACK_LARGE     (6144)
// Dành cho các tác vụ ngốn RAM đặc thù như OTA Update hoặc kết nối HTTPS/TLS
#define DF_TASK_STACK_MAX       (8192)
...
// thời gian chờ giữa các lệnh điều khiển Relay để tránh xung đột cơ khí (cửa cuốn, cửa cổng)
#define DF_INTERLOCK_DELAY_MS   (100)
// thời gian chờ kết nối mạng (WiFi, MQTT, UDP) trước khi báo lỗi
#define DF_NETWORK_TIMEOUT_MS   (5000)
// thời gian chờ kết nối WiFi trước khi báo lỗi
#define DF_WIFI_CONNECT_TIMEOUT_MS  (120000)
// Thời gian trước khi thiết bị restart
#define DF_DEVICE_BEFORE_RESTART (1000)
...
typedef enum {
    ...
    E_APP_STATUS_SUCCESS = 0,
    // hàm thực thi thành công nhưng không có thay đổi trạng thái (ví dụ: lệnh mở cửa khi cửa đã mở)
    E_APP_STATUS_NO_CHANGE,
    // hàm thực thi thất bại vì thiết bị đang bận
    E_APP_STATUS_BUSY,
    // hàm thực thi thất bại vì lỗi không xác định
    E_APP_STATUS_ERROR,
    // hàm thực thi thất bại vì timeout
    E_APP_STATUS_TIMEOUT,
    // hàm thực thi thất bại vì tham số đầu vào không hợp lệ
    E_APP_STATUS_INVALID_PARAM,
    // hàm thực thi thất bại vì thiết bị chưa được khởi tạo
    E_APP_STATUS_NOT_INITIALIZED,
    // hàm thực thi thất bại vì tính năng chưa được hỗ trợ
    E_APP_STATUS_NOT_SUPPORTED,
    // hàm thực thi thất bại vì không tìm thấy thiết bị/phần tử
    E_APP_STATUS_NOT_FOUND,
    // hàm thực thi thất bại vì thiết bị chưa sẵn sàng
    E_APP_STATUS_NOT_READY,
    // hàm thực thi thất bại vì thiết bị/phần tử đã tồn tại
    E_APP_STATUS_ALREADY_EXISTS,
    // hàm thực thi thất bại vì hết bộ nhớ
    E_APP_STATUS_OUT_OF_MEMORY,
    // hàm thực thi thất bại vì lỗi nội bộ
    E_APP_STATUS_INTERNAL_ERROR,
    // hàm thực thi thất bại vì lỗi không xác định
    E_APP_STATUS_UNKNOWN
} e_app_status_t;
...
// sau này chỉ cần gọi DF_RETURN_IF_ERROR(func(...)) thay vì viết 3 dòng kiểm tra lỗi như trên, giúp code gọn hơn và dễ đọc hơn
